...
df <- read.csv('../data/processed/NYCQ_CCA_score_revision_r2z_4_0.4_0.5.csv', header = TRUE, sep = ',')
df <- read.csv('../data/processed/NYCQ_CCA_score_extensiveclean_4_0.1_0.3.csv', header = TRUE, sep = ',')
...
for(i in c(1:length(colnames(Y)))){
  ...
  #     file="../reports/revision/Yeo7_paraest_extensiveclean.txt",sep="\t", append = TRUE)
  # cat("\n",
  #     file="../reports/revision/Yeo7_paraest_extensiveclean.txt",sep="\t", append = TRUE)
  cat(paraest$coefficients[,7],
      file="../reports/revision/Yeo7_paraest_p_revision.txt",sep="\t", append = TRUE)
  cat("\n",
      file="../reports/revision/Yeo7_paraest_p_revision.txt",sep="\t", append = TRUE)
